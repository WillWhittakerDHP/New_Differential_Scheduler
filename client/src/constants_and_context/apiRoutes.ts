// src/constants/apiRoutes.ts

export const API_ROUTES = {
  services: '/internal/admin/serviceTypes',
  additionalServices: '/internal/admin/additionalServiceTypes',
  dwellingAdjustments: '/internal/admin/dwellingAdjustmentTypes',
  availabilityOptions: '/internal/admin/availabilityTypes',
};

export const ADMIN_ROUTES = {
      serviceTypes: '/internal/admin/serviceTypes',
      additionalServiceTypes: '/internal/admin/additionalServiceTypes',
      dwellingAdjustmentTypes: '/internal/admin/dwellingAdjustmentTypes',
      availabilityOptionTypes: '/internal/admin/availabilityOptionTypes',
};
